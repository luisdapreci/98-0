...
**Existing evidence is partial:** Current validation passes 103 engine tests, production build/typechecking and 46 checked-in desktop/320px Playwright checks covering mode selection/locking, full six-pick HI IQ and Daily drafts, exact Mid IQ/HI IQ browser parity, No IQ season/playoffs, postseason presentation, rivalries, audio, Daily deadlines/practice/persistence and recovery. Wider drafting variants, accessibility/performance checks and human playtests remain open. The [release scorecard](../release/RELEASE_SCORECARD.md) retains failed or unmeasured replacement-release gates; passing functional checks are not mode-balance or full-product acceptance. Commands are documented in [the README](../../README.md).
