...
**Social scope (2026-09-17):** Sharing results with friends is the current social direction, using Phase 7 image cards, plain text, downloads and browser share/copy actions. Accounts, server-verified attempts and public leaderboards are deferred to Phase 8 as an optional future improvement, not current release requirements. The ranked-attempt rules below are retained for that future scope. The local single-attempt update is not yet deployed; existing saves remain readable. See [current status](../STATUS.md).
