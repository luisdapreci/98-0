...
This [scoring-core balance release](MID_IQ_CORE_RELEASE.md) gives complementary star-led rosters a credible qualification chance. All five starters retain positive offensive contribution; the core is selected by contribution, not player identity or position. Existing `season-1` and `season-2` runs retain the original five-player average. All subsequent usage, spacing and game-context calculations remain unchanged.
...
| --- | --- | --- |
| **Daily Seeded Run** | Shared UTC challenge and offer priorities, with deterministic legal fallbacks. Launch mode is Mid IQ, selected by the user on 2026-09-16 and pinned alongside seed/rules. Local-only first attempts and practice are implemented; different IQ modes cannot share a ranking. | Local challenge; trusted competition remains Phase 8. |
...
* **Unique calendar (implemented 2026-09-16):** New `daily-2` runs use the approved 28 distinct themes in [the versioned calendar](../src/engine/daily-calendar.ts), one per UTC date from **September 16 through October 13, 2026**. All use Mid IQ and one principal restriction: a fixed coach, allowed eras, allowed franchises, or an exact fixed player/slot. No theme repeats with a new seed, and the calendar does not loop. Dates beyond it require a new published calendar; until then new Daily entry is unavailable, while existing runs remain resumable. See [implementation and verification](PHASE6_PROGRESS.md).
* Fixed-coach days provide only that coach to sign; other days retain three offers. Era/franchise restrictions apply throughout rolls, rerolls and picks. Fixed-player days begin with that exact record locked in its assigned slot and five picks remaining; duplicate-person protection applies across all franchise/era versions. Other draft and simulation rules are unchanged. Every published pool passes a conservative distinct-player position-coverage guarantee for completion after any legal choices. Single-era days keep an unavailable era reroll unspent rather than converting it into a different token.
...
**Status:** Complete for local scope, 2026-09-16. Mid IQ is the selected Daily launch mode. [Implementation and validation](PHASE6_PROGRESS.md). **Depends on:** Phases 3–5. **Rules adopted in:** §4. **Spec:** §2, §4.
...
- [ ] P8.3 Commit one ranked attempt per account/date server-side before revealing coach offers; enforce challenge-day start and server receipt before the 24-hour post-day grace deadline. Support idempotent retries/resumes, input limits, and rate limits; abandonment cannot reset an attempt. Late/practice results remain local and unranked, and resumes retain pinned versions.
