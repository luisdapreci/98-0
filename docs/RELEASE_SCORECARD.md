...
| E1: Release reliability | Candidate-specific tests, typecheck, build and repeatable desktop/mobile journey checks pass; saves and seeded replay remain compatible. | **Partial.** Build/typechecking, 98 engine tests and 26 desktop/mobile browser checks pass, including three-mode selection, a full six-pick HI IQ journey, exact Mid IQ/HI IQ results, No IQ playoffs, saved recovery and postseason presentation. Wider drafting variants and remaining accessibility/performance checks are not covered; no mode-balance or replacement-candidate final acceptance is implied. |
