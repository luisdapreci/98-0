...
The full product goal remains a championship run, with **98-0 meaning exactly 82-0 plus 16-0**. Play-in, playoff series and the championship journey are not implemented. Title frequency and perfect-run rarity remain **blocked pending postseason implementation and baseline measurement**; do not invent a numerical 98-0 target now. Other IQ modes, Daily, sharing and online features require their own enabled-scope acceptance.
...
| E1: Release reliability | Candidate-specific tests, typecheck, build and repeatable desktop/mobile journey checks pass; saves and seeded replay remain compatible. | **Partial.** Latest engine suite: 76 passing tests and clean typecheck. No candidate-specific final build/browser acceptance yet. |
...
| Decision / accepted limitations / release version | Original research cycle exhausted and integration cycle 1 stopped. User separately requested higher usage tolerance and historical postseason entry: local season-5 preview uses 135% base usage and 40-win entry for new runs, with historical players and older saves preserved. No full replacement-release sign-off or further turnover-model cycle approved. Fresh-family, finalist-policy and player tests remain unused. |
