...
import { HISTORICAL_ENTRY_ENGINE_VERSION, QUALIFICATION_45_ENGINE_VERSION, STRICT_USAGE_ENGINE_VERSION } from './engine-versions.ts';
...
export function qualificationWinsForEngine(engineVersion: string): number {
  if (engineVersion === QUALIFICATION_45_ENGINE_VERSION) return 45;
...
export function seasonForQualification(season: SeasonResult, engineVersion: string): SeasonResult {
  if (!HISTORICAL_ENTRY_VERSIONS.includes(engineVersion) && engineVersion !== QUALIFICATION_45_ENGINE_VERSION) return season;
